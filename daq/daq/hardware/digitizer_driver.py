...
class DigitizerDriver(ABC):
    ...
    def _resolve_frequency_hz(self, cfg: dict) -> int:
        ...
        if self.default_frequency_hz is None:
            raise ValueError(
                f"{self.model_name}: no default sampling frequency defined"
            )
...
class X742Driver(DigitizerDriver):
    ...
    group-enable mask rejects bits 0x4/0xF"""
    ...
    def set_enable_mask(self, dev, mask: int) -> None:
        ...
        for g in range(self.n_groups):
            group_channels = ((1 << self.channels_per_group) - 1) << (
                g * self.channels_per_group
            )
    ...
    def configure_post_trigger(self, dev, cfg: dict) -> None:
        percent = max(
            ...
            min(
                100,
                int(cfg.get("post_trigger_size", self.post_trigger_default_percent)),
            ),
        )
    ...
    def configure_triggers(self, dev, cfg: dict) -> None:
        ...
        for item in cfg.get("groups", []):
            ...
            if not 0 <= g < self.n_groups:
                raise ValueError(
                    f"DT5742: group {g} out of range (0..{self.n_groups - 1})"
                )
            if "dc_offset" in item:
                ...
                for ch in self.enabled_channels:
                    if (
                        g * self.channels_per_group
                        <= ch
                        < (g + 1) * self.channels_per_group
                    ):
                        self._caen_call(dev.set_channel_dc_offset, ch, code)
            if "fast_trigger_threshold" in item:
                self._caen_call(
                    dev.set_group_fast_trigger_threshold,
                    g,
                    int(item["fast_trigger_threshold"]),
                )
        offset = cfg.get("dc_offset")
        if offset is not None:
            code = int(offset)
        ...
        if cfg.get("fast_trigger_mode") is not None:
            self._caen_call(
                dev.set_fast_trigger_mode, TriggerMode(cfg["fast_trigger_mode"])
            )
        if cfg.get("fast_trigger_digitizing") is not None:
            self._caen_call(
                dev.set_fast_trigger_digitizing, EnaDis(cfg["fast_trigger_digitizing"])
            )
    ...
    def extract(self, evt):
        ...
        for g_idx, group in enumerate(evt.data_group):
            ...
            group_wf = [
                np.array(group.data_channel[j], dtype=np.float32)
...
class X743Driver(DigitizerDriver):
    ...
    def configure_triggers(self, dev, cfg: dict) -> None:
        ...
        for ch in self.enabled_channels:
            ...
            if ch in thresholds:
                self._caen_call(
                    dev.set_channel_trigger_threshold, ch, int(thresholds[ch])
                )
        offset = cfg.get("dc_offset")
        if offset is not None:
            code = int(offset)
        ...
        if cfg.get("test_pulses"):
            for ch in self.enabled_channels:
                self._caen_call(
                    dev.enable_sam_pulse_gen, ch, 0, SAMPulseSourceType.SOFTWARE
                )
        ...
        for g_idx, group in enumerate(evt.data_group):
            ...
            for j, arr in enumerate(group.data_channel):
                ...
                if ch in self.enabled_channels:
                    waveforms[self.enabled_channels.index(ch)] = np.asarray(
